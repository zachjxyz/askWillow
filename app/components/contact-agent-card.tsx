...
export function ContactAgentCard({ input, onRespond }: ContactAgentCardProps) {
  ...
  if (responded) {
    return (
      <div className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-xs ${
        ...
        <span>{responded === "approved" ? "Contact request submitted" : "Contact request cancelled"}</span>
      </div>
    );
  }
      ...
      <div className="flex gap-2">
        <button
          ...
          onClick={() => {
            ...
            onRespond(true, message || undefined);
          }}
        ...
        <button
          ...
          onClick={() => {
            ...
            onRespond(false, message || undefined);
          }}
          className="cursor-pointer rounded-lg border border-zinc-300 bg-white px-3 py-1.5 text-xs font-medium text-zinc-600 transition-colors hover:bg-zinc-100 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
        >
          No thanks
        </button>
      </div>
